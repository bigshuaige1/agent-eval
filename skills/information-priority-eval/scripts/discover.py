...
SIGNAL = re.compile(
    ...
    r"(?:重点|关键)(?:不是|在于|是)|"
    r"(?:不需要|不对|不准确|不是这样|你应该|你需要|请不要|需要补充|要全面|尽可能)|"
    r"(?:missed|omitted|overlooked|forgot) (?:the |a )?(?:point|requirement|constraint|detail)|"
    r"(?:not what I asked|missed the point|you forgot|you ignored|actually|instead|please don't)", re.I)
...
def candidates_from_text(text, source, locator, previous_user="", previous_assistant=""):
    ...
    for match in SIGNAL.finditer(text):
        yield {"id": hashlib.sha256(text.encode()).hexdigest()[:16],
               ...
               "previous_user": SECRET.sub("[REDACTED]", previous_user),
               "previous_assistant": SECRET.sub("[REDACTED]", previous_assistant),
        ...
        for text, number, role in records:
            ...
            if role == "user" and text.strip():
                ...
                entry = {"id": digest, "source": source, "locator": locator,
                         "message": SECRET.sub("[REDACTED]", text),
                         "previous_user": SECRET.sub("[REDACTED]", previous_user) if source == "codex_user_session" else "",
                         "previous_assistant": SECRET.sub("[REDACTED]", previous_assistant) if source == "codex_user_session" else ""}
...
def main():
    ...
    parser.add_argument("--path", type=Path, action="append", default=[],
                        help="additional Markdown, text, or JSONL file/directory")
    ...
    items, archive, consumed = scan(paths, args.max_mib * 1024 * 1024, args.max_candidates)
