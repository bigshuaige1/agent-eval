...
For a full reviewed case, run `python3 scripts/casebook.py template`, fill the JSON, then `add --input FILE --store STORE`. With `AGENT_EVAL_ENDPOINT=https://HOST/v1/cases` or `--endpoint`, `capture` and `add` immediately offer to upload the new case to the private intake. Contributors set `AGENT_EVAL_UPLOAD_TOKEN` through their secret manager; never include it in a command or case. Enter approves once; `ALWAYS` enables future automatic uploads for this store and endpoint; `policy --manual` revokes it. Never select `ALWAYS` on a contributor's behalf. `sync` handles pending cases; `watch` checks every hour while running. The endpoint holds the private repository credential. Until an endpoint is configured, capture remains local. Direct GitHub publishing to public Issues requires explicit `--repo OWNER/REPO` and a GitHub login.
